...
outlier_pic_vect <- function(nb_outliers,vect_min_frq,vect_max_frq,vect_sd_frq){
  ...
  for(i in 1:nb_outliers){
    ...
    pics_out <- append(pics_out,vect_min_frq[x])
    pics_out <- append(pics_out,vect_max_frq[-x])
...
test <- outlier_pic_vect(8,as.matrix(data_mean_sd[1,-1]),as.matrix(data_mean_sd[2,-1]),as.matrix(data_mean_sd[3,-1]))
...
outlier_spectre <- function(tab_spectre){
  
  part_gen <- tab_spectre
  for(i in 1:length(colnames(tab_spectre))){
...
```{r}
write.table(test , file="R:/R&D/2-Anis Tanich/R-LNE/Analyse Machine Learning/reunion  RUS 13-07-22 NASSIM/outlier 24 parts ( pics et spectres)/outlier4_pics_8parts.xls", quote=TRUE,dec=",", row.names=FALSE, col.names=TRUE, sep ="\t", qmethod = c("escape"))
```
